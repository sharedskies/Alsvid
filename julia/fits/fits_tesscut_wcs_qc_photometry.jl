...
# fits_tesscut_wcs_qc_photometry
#
#   Photometry through the time-axis of a selected tesscut fits file
#     Data selected by quality flag
#
#   Explicit inputs:
#
...
#   2025-09-21 Version 1.0.1
#     Added notes from Google Gemini explaining TESSCUT BJD times
#   2025-11-02 Version 1.1
#     Corrected error in background subtraction within appphot
#     Consistent use of median annulus signal for the background estimate
#     Commented unused calculations of mean and standard deviations
#   2025-11-04 Version 1.1.1
#     Updated documentation inline
#   2026-01-29 Version 1.1.2
#     Corrected r3sq condition in mask
...
#     TIME: This column contains the timestamp for each data frame. 
#     The unit is #  typically specified in the header as 'BJD - 2457000, days'.
#     This is the BTJD.

#     TIMECORR: This column provides the barycentric correction in days.

#   In the Binary Table Header (HDU 1 Header):

#     BJDREFI: This keyword provides the integer part of the BJD reference time.
#     For TESS data, this is typically 2457000.

#     BJDREFF: This keyword gives the fractional part of the BJD reference time,
#     which is usually 0.

#     TIMEUNIT: This keyword specifies the unit of the TIME column, which is 'd'
#     for days.

#   Calculation Steps
#     To get the BJD for each frame, you simply need to add the BJDREFI value to
#     the TIME value from the data table.

#   The formula is as follows:

#     BJD=TIME+BJDREFI

#     Since TIME is already the Barycentric TESS Julian Date (BTJD), and BTJD is
#     defined as BJD - 2457000.0, adding the BJDREFI of 2457000.0 to the TIME
#     column effectively gives you the BJD.

###

using FITSIO               # For FITS management
using WCS                  # For World Coordinate System in FITS
using Statistics           # For median, mean, and standard deviation of arrays
...
#   r_phot    => radius of photometry in pixels
#   r_inner   => inner radius of background annulus in pixels
#   r_outer   => out radius of backbround annulus in pixels
...
# Return arrays of header strings, images, times, quality
...
function build_tesscut_stacks(fits_file)
  ...
  ntimes = length(times)
  quality = FITSIO.read(f[2], "QUALITY")
  ...
  return header, images, times, quality
...
# Write a 2-column data file of  x and y vectors if quality flag is 0
# Requires using DelimitedFiles at top level

function write_xy_quality_data(out_file, x_array, y_array, q_array) 
  ...
  for i in 1:nxy
    if q_array[i] == 0
      push!(qx_array, x_array[i])
      push!(qy_array, y_array[i])
    end
  end  
  
  open(out_file, "w") do io
    writedlm(io, [qx_array qy_array])
  end
...
function process_images()
  ...
  header, images, times, quality = build_tesscut_stacks(fits_file)
  ...
  for j in 1:ntargets
    ...
    write_xy_quality_data(out_file, times, signals[j], quality)
