...
"""
   ...
   to find the TDB for a given UTC.
   
   This version is valid only for the most recent leap second update,
   which can be recoded by changing the value in the code.  It is limited by the
   the validity range of the DE440t and is intended for the precision
   conversion of contempory observation times.
...
if length(ARGS) != 1
  println("Compute the TDB for an JD UTC")
  println("Use jd  on the command line.")
  exit()
end
   
jd_utc = parse(Float64, ARGS[1])
dt_tai = leapseconds/86400.0
dt_tt = 32.184/86400.0
jd_tt = jd_utc + dt_tai + dt_tt 
...
print("TDB-TT (s) ", dt_tdb_sec,"\n")
dt_tdb = dt_tdb_sec/86400.0
print("Rate of change (s/d) ", -1.0*jpl_state[2][1],"\n")
jd_tdb = jd_tt + dt_tdb
print("TDB ", jd_tdb,"\n")
println(" ")
