...
"""
  ...
  Find the barycentric dynamical time TDB from JD UTC for given observation
  This version is targets outside the solar system 
...
if len(sys.argv) > 2:
  ...
  print(" ")
  sys.exit("Usage: tdb_stellar.py request_file.txt\n")
  exit()
  ...
  print("rv =          measured approximate radial velocity [m/s] where the default is 0.")
...
# 'rv'           approximate radial velocity [m/s] where the default is 0.
...
print("Barycentric dynamical times: ")
print ("")

if diagnostics_flag:
  print(tdbs)

for n in range(len(tdbs[0])):
  print(jd_list[n], "  ", tdbs[0][n])
