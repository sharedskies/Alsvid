...
"""
  ...
  Sliding median filter a stack of images
  
    Return a stack of filtered images
...
if len(sys.argv) == 1:
  print(" ")
  print("Usage: fits_sliding_median.py  n_median outprefix infile1.fits infile2.fits ...   ")
  print(" ")
  sys.exit("Filter a stack of  fits images with a sliding median\n ")
elif len(sys.argv) >=4:
...
# This block is used for detrending.  See fits_detrend_sliding_median.py.
#
# Find a median for the entire stack  to scale the detrended output
#
# median_image= np.median(working_stack, axis=0)
#
# There are two stacks in memory.  Vectorize and do stack math in place here.
#
...
for i in range(n_in):
  ...
  out_hdr['history'] = 'Sliding %d median filter' %(n_median,)
