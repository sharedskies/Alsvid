...
"""
   ...
   Input a FITS Bayer-masked image file
   Denoise the original color slices individually
   Balance colors
   Write a linearly scaled color png image
...
# Denoise and scale an image for output in a color-balanced PNG format

def color_balance(inimage):

  # Denoise the image
    
  inimage = skr.denoise_tv_chambolle(inimage) 
...
if len(sys.argv) != 3:
  ...
  print(" ")
  sys.exit("Create a denoised linearly-scaled color-balanced png image\n")
